<?php
  ...
  require_once( 'external/fibb-resize.php' );
...
function fibb_fetch_responsive_images($content)
{
  ...
  foreach (@$results[0] as $result)
  {
    if (strpos($result, 'wp-content/') !== false)
    {
      ...
      $sources = array();
      $sources['wide'] = array('src' => MAIN_URL.resize_image($src, 'wide', false, 90), 'width' => $size[0], 'height' => $size[1]);
      $sources['desktop'] = array('src' => MAIN_URL.resize_image($src, 'desktop', false, 90), 'width' => $size[0], 'height' => $size[1]);
      $sources['tablet'] = array('src' => MAIN_URL.resize_image($src, 'tablet', false, 90), 'width' => $size[0], 'height' => $size[1]);
      $sources['mobile'] = array('src' => MAIN_URL.resize_image($src, 'mobile', false, 90), 'width' => $size[0], 'height' => $size[1]);

      $sources['wide-retina'] = array('src' => MAIN_URL.resize_image($src, 'wide', true, 75), 'width' => $size[0], 'height' => $size[1]);
      $sources['desktop-retina'] = array('src' => MAIN_URL.resize_image($src, 'desktop', true, 75), 'width' => $size[0], 'height' => $size[1]);
      $sources['tablet-retina'] = array('src' => MAIN_URL.resize_image($src, 'tablet', true, 75), 'width' => $size[0], 'height' => $size[1]);
      $sources['mobile-retina'] = array('src' => MAIN_URL.resize_image($src, 'mobile', true, 75), 'width' => $size[0], 'height' => $size[1]);

      $content = str_replace($result, '<span class="responsive-image"><noscript data-base="'.MAIN_URL.'" data-src=\''.json_encode($sources).'\'>'.$result.'</noscript></span>', $content);
    }
  }
